...
```shell
balaniaf@node-1:~$ touch config.txt
balaniaf@node-1:~$ nano config.txt 
balaniaf@node-1:~$ kubectl create configmap db-config --from-env-file=config.txt
configmap/db-config created
balaniaf@node-1:~$ kubectl run backend --image=nginx --restart=Never -o yaml --dry-run > pod.yaml
balaniaf@node-1:~$ vim pod.yaml 
balaniaf@node-1:~$ k get configmap
NAME        DATA   AGE
db-config   2      6m27s

balaniaf@node-1:~$ cat pod.yaml 
apiVersion: v1
kind: Pod
metadata:
  creationTimestamp: null
...
spec:
  ...
  restartPolicy: Never
status: {}
balaniaf@node-1:~$ k create -f pod.yaml 
pod/backend created
balaniaf@node-1:~$ k get pod
NAME      READY   STATUS    RESTARTS   AGE
backend   1/1     Running   0          4s
balaniaf@node-1:~$ k exec backend -it -- /bin/sh
# env
DB_URL=localhost:3306
