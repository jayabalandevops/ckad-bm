...
```shell
devuser@node-1:~$ touch config.txt
devuser@node-1:~$ nano config.txt 
devuser@node-1:~$ kubectl create configmap db-config --from-env-file=config.txt
configmap/db-config created
devuser@node-1:~$ kubectl run backend --image=nginx --restart=Never -o yaml --dry-run > pod.yaml
devuser@node-1:~$ vim pod.yaml 
devuser@node-1:~$ k get configmap
NAME        DATA   AGE
db-config   2      6m27s

devuser@node-1:~$ cat pod.yaml 
apiVersion: v1
kind: Pod
metadata:
  creationTimestamp: null
...
spec:
  ...
  restartPolicy: Never
status: {}
devuser@node-1:~$ k create -f pod.yaml 
pod/backend created
devuser@node-1:~$ k get pod
NAME      READY   STATUS    RESTARTS   AGE
backend   1/1     Running   0          4s
devuser@node-1:~$ k exec backend -it -- /bin/sh
# env
DB_URL=localhost:3306
